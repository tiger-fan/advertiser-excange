...
dojo.declare("extras.ClusterLayer", esri.layers.GraphicsLayer, {
  constructor: function(options) {
    ...
    this._singleSym = options.singleSymbol || new sms("circle", 1, null, new dojo.Color("#fff"));//new esri.symbol.PictureMarkerSymbol("http://static.arcgis.com/images/Symbols/Shapes/OrangePin1LargeB.png", 20, 20).setOffset(0, 0);/*new sms("circle", 12, new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID,
   // new dojo.Color([255,255,255]), 1), new dojo.Color([255,0,0,0.6]));*/
    ...
    if (e.graphic.attributes.clusterCount > 1)
    {
      for ( var i = 0, il = this._clusterData.length; i < il; i++) {
        if ( e.graphic.attributes.clusterId == this._clusterData[i].attributes.clusterId &&
             this._clusterData[i].visible == true ) {
          this._map.centerAndZoom(e.graphic.geometry, 14);
          return;
        }
      }
    }
  ...
  */
  ...
  _clusterGraphics: function() {
    ...
    for ( var j = 0, jl = this._clusterData.length; j < jl; j++ ) {
      ...
      if (this._map != null && this._map.getZoom() < 13)
      {
        for ( var i = 0; i < this._clusters.length; i++ ) {
          var c = this._clusters[i];
